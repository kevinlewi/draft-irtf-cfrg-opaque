...
~~~
 Client (password, creds)            Server (server_private_key, server_public_key, oprf_seed)
 --------------------------------------------------------------------
 (request, blind) = CreateRegistrationRequest(password)
            ...
            (response, oprf_key) = CreateRegistrationResponse(request, server_public_key, cred_identifier, oprf_seed)
...
masking_key
: A key used by the server to preserve the confidentiality of the envelope during login
...
~~~
CreateRegistrationResponse(request, server_public_key, cred_identifier, oprf_seed)

Input:
- request, a RegistrationRequest structure
- server_public_key, the server's public key
- cred_identifier, an identifier that uniquely represents the credential being
  registered
...
Steps:
1. (oprf_key, _) = DeriveKeyPair(Expand(oprf_seed, concat("oprf key", cred_identifier), Nok))
...
~~~
 Client (password)             Server (server_private_key, server_public_key, oprf_seed, credential_file)
 --------------------------------------------------------------------
 (request, blind) = CreateCredentialRequest(password)
    ...
    response = CreateCredentialResponse(request, server_public_key, credential_file, cred_identifier, oprf_seed)
...
struct {
    SerializedElement data;
    opaque encrypted_response[Npk + Nsk + Nh + 33];
} CredentialResponse;
~~~
...
credential_file record for the client exists (corresponding to a properly registered client), or
it was never created (corresponding to a client that has yet to register).

In the case of a credential_file record that exists, the server invokes the following function to
produce a CredentialResponse:

~~~
CreateCredentialResponse(request, server_public_key, credential_file, cred_identifier, oprf_seed)

Input:
- request, a CredentialRequest structure
- server_public_key, the public key of the server
- credential_file, an instance of RegistrationUpload which is the server's
  output from registration (see {{credential-file}})
- cred_identifier, an identifier that uniquely represents the credential being
  registered
- oprf_seed, the server-side seed of Nh bytes used to generate an oprf_key
...
1. (oprf_key, _) = DeriveKeyPair(Expand(oprf_seed, concat("oprf key", cred_identifier), Nok))
2. Z = Evaluate(oprf_key, request.data)
3. encryption_nonce = random(32)
4. credential_response_pad = Expand(credential_file.masking_key,
     "CredentialResponse pad", Npk + Nsk + Nh + 33)
5. encrypted_response = xor(credential_response_pad, concat(server_public_key, credential_file.envelope))
6. Create CredentialResponse response with (Z, encryption_nonce, encrypted_response)
7. Output response
~~~

In the case of a credential_file record that does not exist, the server invokes the CreateCredentialResponse
function where the credential_file parameter is configured so that:
- credential_file.masking_key is set to a random byte string of length 32, and
- credential_file.envelope is set to the byte string consisting only of zeros, of length Nsk + Nh + 33

Note that the responses output by either scenario are indistinguishable to an adversary
that is unable to guess the registered password for the client corresponding to cred_identifier.
...
Steps:
1. y = Finalize(password, blind, response.data)
2. prk = Extract(salt=0, Harden(y, params))
3. masking_key = Expand(prk, "MaskingKey", Nh)
4. credential_response_pad = Expand(credential_file.masking_key,
     "CredentialResponse pad", Npk + Nsk + Nh + 33)
5. concat(server_public_key, envelope) = xor(credential_response_pad, response.encrypted_response)
